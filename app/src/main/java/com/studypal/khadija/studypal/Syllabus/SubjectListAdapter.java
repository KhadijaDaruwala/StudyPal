...
public class SubjectListAdapter extends CursorAdapter {
    ...
    public SubjectListAdapter(Context context, Cursor cursor, int flags, AdapterCallback callback) {
        super(context, cursor, 0);
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        View view = LayoutInflater.from(context).inflate(R.layout.listview_subject, parent, false);
        className = (TextView) view.findViewById(R.id.subjectName);
    ...
    public void bindView(View view, final Context context, final Cursor cursor) {
        ...
        className.setText(cursor.getString(cursor.getColumnIndex(DatabaseSubject.COLUMN_NAME)));


        /* Setting the icon, displaying how many assignments are in the class. */
        if (cursor.getInt(cursor.getColumnIndex(DatabaseSubject.COLUMN_QUANTITY)) <= 9)
            numIcon.setImageResource(icon[cursor.getInt(cursor.getColumnIndex(DatabaseSubject.COLUMN_QUANTITY))]);
        else
            numIcon.setImageResource(R.drawable.num_plus);

        ImageView boximg = (ImageView) view.findViewById(R.id.box);
        boximg.setImageResource(box[cursor.getInt(cursor.getColumnIndex(DatabaseSubject.COLUMN_COLOR))]);
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP: {
                        ...
                        if (clickDuration < MAX_CLICK_DURATION && dx < MAX_CLICK_DISTANCE && dy < MAX_CLICK_DISTANCE) {
                            ...
                            mAdapterCallback.onMethodCallback(1, cursor.getString(cursor.getColumnIndex(DatabaseSubject.COLUMN_NAME)));
                            ...
                            if (offset > DEFAULT_THRESHOLD) {
                                if (!called) {
                                 /* When a class is swiped from left to right, delete the class, and its assignments */
                                    cursor.moveToPosition(pos);
                                    Subject mSubject = new Subject(cursor.getString(cursor.getColumnIndex(DatabaseSubject.COLUMN_NAME)),
                                            cursor.getInt(cursor.getColumnIndex(DatabaseSubject.COLUMN_QUANTITY)),
                                            cursor.getInt(cursor.getColumnIndex(DatabaseSubject.COLUMN_COLOR)));

                                                                              /* Deleting the class. */
                                                            /*DatabaseSubject db = new DatabaseSubject(context, null, null, 1);
                                                            db.deleteSubject(mSubject.getSubjectName());
                                                            db.close();*/

                                  /* Deleting the associating assignments */
                                    DatabaseChapter dbAssignment = new DatabaseChapter(context, null, null, 1);
                                    Cursor c = dbAssignment.getAssignments(mSubject.getSubjectName());
                                    if (c.moveToFirst()) {
                                        do {
                                            dbAssignment.deleteAssignment(c.getString(c.getColumnIndex(DatabaseChapter.COLUMN_NAME)));
                                        } while (c.moveToNext());
                                    }
                                    c.close();
                                    dbAssignment.close();
                                    Toast.makeText(context, mSubject.getSubjectName() + " deleted", Toast.LENGTH_SHORT).show();
                                    called = true;
                                    mAdapterCallback.onMethodCallback(0, mSubject.getSubjectName());
                                }
                               ...
                               /* Open class edit dialog. */
                                    mAdapterCallback.onMethodCallback(2, mSubject.getSubjectName());
                                }
                            }
                        }
                        break;
